class WorksController < ApplicationController
  ...
  def edit
      if @work.user_id != @login_user.id
        flash[:status] = :failure
        flash[:result_text] = "You must be the owner of #{@work.title} to edit this #{@media_category.singularize}."
  ...
  def destroy
    if @work.user_id != @login_user.id
      flash[:status] = :failure
      flash[:result_text] = "You must be the owner of #{@work.title} to delete this #{@media_category.singularize}."
      redirect_to work_path(@work.id)
    else
      @work.destroy
      flash[:status] = :success
